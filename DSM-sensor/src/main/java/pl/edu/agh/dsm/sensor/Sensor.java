...
public class Sensor {
	...
	private static final ScheduledExecutorService es = Executors
			.newScheduledThreadPool(4); // TODO

	public static void main(String[] args) {
		if (args.length < 3) {
			System.out
					.println("Usage: sensor <monitor address> <resource> <metric> <time ms> [<metric> <time ms> [ ... ]]");
		...
		try {
			startSensors(args, sensorConfiguration, new SensorSocket(ip,
	...
	private static void startSensors(String[] args,
			SensorConfiguration sensorConfiguration, SensorSocket sensorSocket) {
		...
		for (int i = 2; i < args.length; i += 2) {
			...
			try {
				SensorThread thread = new SensorThread(
						factory.createMonitoredResource(args[i]),
						sensorConfiguration, sensorSocket);
				es.scheduleAtFixedRate(thread, 0,
						Integer.parseInt(args[i + 1]), TimeUnit.MILLISECONDS);
			} catch (MonitoringException ex) {
				log.error(
						"MonitoringException caught while starting sensor for metric "
								+ args[i] + ": ", ex);
			} catch (IndexOutOfBoundsException ex) {
				log.error(
						"IndexOutOfBoundsException caught while starting sensor for metric "
								+ args[i] + ": ", ex);
			} catch (NumberFormatException ex) {
				log.error(
						"NumberFormatException caught while starting sensor for metric "
								+ args[i] + ": ", ex);
			}
		}
	}
