...
public class SensorThread implements Runnable {
    ...
    public SensorThread(MonitoredResource resource, SensorConfiguration configuration, SensorSocket sensorSocket) {
    ...
    public void run() {
        ...
        try {
            ...
            log.trace("Successfully checked value of resource " + resource.getId() + ", got result: " + result);
        } catch (MonitoringException ex) {
            if (ex.isFatal()) {
                log.error("MonitoredResource failure, most likely due to malformed configuration,"
                        + " see MonitoringException message and stacktrace for cause: ", ex);
                throw new RuntimeException("thread is kill. no.");
            } else {
                log.error("Non-fatal MonitoringException caught while executing resource "
                        + "value update for sensor " + resource.getId() + ": ", ex);
            }
        }
    }

    private void announceNewValue(String resourceId, String metric, Double resourceValue) {
        log.trace("Sending update to monitor server (resource id: " + resourceId + ")");
